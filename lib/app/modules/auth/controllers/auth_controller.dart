...
class AuthController extends GetxController {
  ...
  void login() async {
    ...
    if (loginFormKey.currentState.validate()) {
      ...
      try {
        ...
        Get.log("Curent user");
        Get.log("Curent user");
        Get.log(currentUser.value.toJson().toString());
  ...
  Future<void> verifyPhone() async {
    try {
      ...
      Get.showSnackbar(Ui.SuccessSnackBar(message: "Registered Successfully"));
  ...
  Future pickImage() async {
    ...
    imagePicker.getImage(source: ImageSource.gallery).then((file) async {
      ...
      currentUser.update((val) {
        ...
        val.avatar=media;
        print(currentUser.value.avatar.icon);
      });
    });
  }
  Future pickProfileImage() async {
    ...
    imagePicker.getImage(source: ImageSource.gallery).then((file) async {
      ...
      Media media=Media();
      currentUser.update((val) {
        val.profileImg=base64Image;

      });
    });
  }
  Future pickCitizenFrontImage() async {
    ImagePicker imagePicker = ImagePicker();
    imagePicker.getImage(source: ImageSource.gallery).then((file) async {
      var bytes = await file.readAsBytes();
      String base64Image = "data:image/jpeg;base64,"+base64Encode(bytes);
      Media media=Media();
      currentUser.update((val) {
        val.citizenshipFront=base64Image;

      });
    });
  }
  Future pickCitizenBackImage() async {
    ImagePicker imagePicker = ImagePicker();
    imagePicker.getImage(source: ImageSource.gallery).then((file) async {
      var bytes = await file.readAsBytes();
      String base64Image = "data:image/jpeg;base64,"+base64Encode(bytes);
      Media media=Media();
      currentUser.update((val) {
        val.citizenshipBack=base64Image;
