...
class ImagesFieldController extends GetxController {
  ...
  Future pickImage(ImageSource source, ValueChanged<String> uploadCompleted) async {
    ...
    PickedFile pickedFile = await imagePicker.getImage(source: source, imageQuality: 80);
    File imageFile = File(pickedFile.path);
    print(imageFile);
    if (imageFile != null) {
      try {
        uploading.value = true;
        var _uuid = await _uploadRepository.image(imageFile, "image");
        uuids.add(_uuid);
      ...
      Obx(() {
        if (controller.uploading.isTrue)
          ...
          return GestureDetector(
            onTap: () async {
              await controller.pickImage(ImageSource.gallery, uploadCompleted);
            },
            child: Container(
              width: 100,
              height: 100,
